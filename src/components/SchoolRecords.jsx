...
function SchoolRecords({ onBack }) {
  ...
  const loadStudentFees = async ()=>{
    const res = await API.get(`/api/fees/filter`,{headers,params:{name:search,className:classFilter,section:sectionFilter,status:statusFilter}});
    setPayments(res.data||[]);
  };
  const loadTeachers=async()=>{const res=await API.get(`/api/admin/teachers`,{headers});setTeachers(res.data)};
  const loadTeacherPayments=async(id)=>{const res=await API.get(`/api/teacher/${id}/payments`,{headers});setTeacherPayments(p=>({...p,[id]:res.data}))};
  const loadExpenses=async()=>{const res=await API.get(`/api/expenses`,{headers});setExpenses(res.data)};
  const loadOverview=async()=>{const res=await API.get(`/api/reports/overview`,{headers});setOverview(res.data)};
  const loadFeeStructures=async()=>{const res=await API.get(`/api/fees/structure`,{headers});setFeeStructures(res.data)};
  const loadTeacherStructures=async()=>{const res=await API.get(`/api/teacher/structure`,{headers});setTeacherStructures(res.data)};

  // ---- Add Handlers ----
  const addFee=async(e)=>{e.preventDefault();await API.post(`/api/fees/payment`,newFee,{headers});setNewFee({studentId:"",feeType:"",month:"",receiptNo:"",amountPaid:"",status:"paid"});loadStudentFees();};
  const addTeacherPayment=async(e)=>{e.preventDefault();await API.post(`/api/teacher/${newPayment.teacherId}/payments`,newPayment,{headers});setNewPayment({teacherId:"",month:"",amount:"",status:"paid"});loadTeacherPayments(newPayment.teacherId);};
  const addExpense=async(e)=>{e.preventDefault();await API.post(`/api/expenses`,newExpense,{headers});setNewExpense({title:"",amount:"",category:"other"});loadExpenses();};
  const addFeeStructure=async(e)=>{e.preventDefault();await API.post(`/api/fees/structure`,newStructure,{headers});setNewStructure({className:"",feeType:"",amount:"",dueDate:""});loadFeeStructures();};
  const addTeacherStructure=async(e)=>{e.preventDefault();await API.post(`/api/teacher/structure`,newTeacherStructure,{headers});setNewTeacherStructure({teacherId:"",salaryType:"fixed",amount:"",month:""});loadTeacherStructures();};
