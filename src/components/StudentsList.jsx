...
function StudentsList({ onBack }) {
  ...
  const loadStudents = async () => {
    try {
      const res = await API.get(import.meta.env.VITE_API_BASE, {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await API.delete(`
`import.meta.env.VITE_API_BASE/api/auth/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingStudent) {
        // Update student
        await API.put('
          `import.meta.env.VITE_API_BASE/api/auth/${editingStudent._id}`,
          formData,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        ...
        // Add student
        await API.post(import.meta.env.VITE_API_BASE, formData, {
          headers: { Authorization: `Bearer ${token}` },
        });
        alert("Student added!");
      }
    ...
    return (
      <div style={{ padding: "20px" }}>
        ...
        {selectedStudent.photo && (
          <img
            src={`import.meta.env.VITE_API_BASE${selectedStudent.photo}`}
            alt="student"
