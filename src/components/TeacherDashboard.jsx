...
function TeacherDashboard({ teacher, onLogout }) {
  ...
  useEffect(() => {
    ...
    API.get(`/api/classes?teacher=${teacher._id}`, {
      headers: { Authorization: `Bearer ${token}` },
    }).then((res) => setClasses(res.data));

    API.get(`/api/tests?createdBy=${teacher._id}`, {
      headers: { Authorization: `Bearer ${token}` },
    }).then((res) => setTests(res.data));

    API.get(`/api/teachers/${teacher._id}/payments`, {
      headers: { Authorization: `Bearer ${token}` },
    }).then((res) => setPayments(res.data));

    // ✅ fetch notices
    API.get(import.meta.env.VITE_API_BASE, {
      headers: { Authorization: `Bearer ${token}` },
    }).then((res) => {
      const all = res.data || [];
