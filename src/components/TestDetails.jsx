...
export default function TestDetails(){
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await API.get(`/api/tests/${id}`, { headers: { Authorization: `Bearer ${token}` } })
      setTest(res.data)
  ...
  const handleAddQuestion = async (e) => {
    ...
    try {
      ...
      const qRes = await API.post('/api/questions', { ...questionForm, options: optionsArr, correctAnswer: Number(questionForm.correctAnswer) }, { headers: { Authorization: `Bearer ${token}` } })
      // attach to test
      await API.put(`/api/tests/${id}`, { questions: [...(test.questions||[]), qRes.data._id] }, { headers: { Authorization: `Bearer ${token}` } })
