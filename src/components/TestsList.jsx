...
export default function TestsList({ onBack }){
  ...
  const load = async () => {
    setLoading(true)
    try {
      const res = await API.get(import.meta.env.VITE_API_BASE, { headers: { Authorization: `Bearer ${token}` } })
      setTests(res.data)
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await API.put('`import.meta.env.VITE_API_BASE/api/tests/${id}`, { deleted: true }, { headers: { Authorization: `Bearer ${token}` } })
      load()
